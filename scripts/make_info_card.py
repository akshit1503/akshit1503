...
ROWS = [
    ("host",),
    ("kv", "Now", "CSE Student @ UPES, Dehradun"),
    ("kv", "Focus", "Full-Stack Dev · AI/ML"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Languages", "Java, C++, JS, TypeScript, Python"),
    ("kv", "Web", "Node.js, Express, React, Next.js"),
    ("kv", "AI / ML", "GenAI, TensorFlow, RAG"),
    ("kv", "Data", "MongoDB, MySQL, Redis, Firebase"),
    ("kv", "Cloud", "AWS, Azure, GCP, Vercel"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "Built & deployed a Stripe payment gateway"),
    ("bul", "Backend lead, Parking Lot Management System"),
    ("bul", "Changemaker, UPES Career Service Dept."),
    ("bul", "UNESCO Special Mention delegate (AMIMUN)"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">akshit@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">akshit</tspan><tspan fill="{MUTED}">@</tspan>'
